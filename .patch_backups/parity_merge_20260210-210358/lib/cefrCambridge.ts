...
export function parseCefrLevel(input: unknown): CefrLevel {
  ...
  // Back-compat: numeric old stage/level mapping
  const n = Number(s);
  if (!Number.isNaN(n)) {
    if (n <= 2) return "A2";
    if (n === 3) return "B1";
  ...
  switch (level) {
    ...
    case "C2":
      return [
        ...
        "Accuracy: near-native control without being unnatural or â€˜thesaurusyâ€™.",
      ].join("\n");
  }
}
  ...
  switch (type) {
    ...
    case "short_message":
      return [
        ...
        "Include: direct purpose; minimal fluff; closing optional.",
      ].join("\n");
    case "email_informal":
      return [
        "Format: Informal email.",
        "Must include: Subject line; greeting; closing line; sign-off.",
      ].join("\n");
    case "email_formal":
      return [
        "Format: Formal email.",
        "Must include: Subject line; formal greeting; polite closing; full name sign-off.",
      ].join("\n");
    case "article":
      return [
        "Format: Article.",
        "Must include: title; clear paragraphs; optional subheadings.",
      ].join("\n");
    case "review":
      return [
        "Format: Review.",
    ...
    case "report":
      return [
        "Format: Report.",
        "Must include: headings (Introduction, Findings, Recommendations).",
      ].join("\n");
    case "essay":
      return [
        "Format: Essay.",
        "Must include: intro with thesis; 2â€“3 body paragraphs; conclusion.",
      ].join("\n");
  }
}
...
export function buildCambridgeConstraints(level: CefrLevel, type: TextType): string {
  ...
  return [
    ...
    `Text type: ${type}`,
    `Register: ${register}`,
    `Target length: ${wt.target} words (acceptable range: ${wt.min}â€“${wt.max})`,
    "",
    ...
    "- STANDARD and SUPPORTED must share ONE answer key; supports are additive only.",
    "- Use plain ASCII punctuation only (use straight apostrophes and straight double quotes).",
  ].join("\n");
}